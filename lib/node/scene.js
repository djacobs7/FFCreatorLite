...
const MIXED_AUDIO_OUTPUT = 'audio'
...
class FFScene extends FFCon {
  ...
  mixAudio(command) {
    ...
    const audioIndexes = this.children.
      filter((child) => child.conf && child.conf.audio).
      map((child) => child.getAId(false))
    if (audioIndexes.length === 0) return;

    this.filters.push(
      {
        filter: 'amix',
        options: { inputs: audioIndexes.length },
        inputs: audioIndexes.map((idx) => `[${idx}]`),
        outputs: `[${MIXED_AUDIO_OUTPUT}]`
      }
    )


    command.outputOptions("-map", `[${MIXED_AUDIO_OUTPUT}]`)
    command.outputOptions(["-c:a", "aac", "-b:a", "128k", "-ar", "44100", "-ac", "2"])
